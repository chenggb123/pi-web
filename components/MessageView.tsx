...
function UserMessageView({ message, entryId, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent }: {
  ...
  const fileBlocks = typeof message.content === "string"
    ? []
    : (message.content as { type: string }[]).filter((b): b is { type: "file"; name: string; mimeType: string; size: number; data: string } => b.type === "file");
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: content ? 8 : 0 }}>
              {fileBlocks.map((file, i) => (
                <div key={i} style={{
                  ...
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 500, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 160 }}>
                      {file.name}
                    </div>

...
function shortenCwd(cwd: string, homeDir?: string): string {
  ...
  const ncwd = norm(cwd);
  const nhome = homeDir ? norm(homeDir) : "";

  // Replace home directory prefix with ~
  const path = (nhome && ncwd.startsWith(nhome)) ? "~" + ncwd.slice(nhome.length) : ncwd;
  const parts = path.split("/").filter(Boolean);

  // Never return just "~" — fall back to full path
  if (parts.length === 0) return ncwd;
  if (parts.length === 1 && parts[0] === "~") return ncwd;

  // Show up to 3 segments before truncating (e.g. ~/pi-cwd/admin)
...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention, workspaceLocked }: Props) {
  ...
  // Sync selectedCwd with prop when workspace is locked
  useEffect(() => {
    if (workspaceLocked && selectedCwdProp && selectedCwd !== selectedCwdProp) {
      setSelectedCwd(selectedCwdProp);
    }
  }, [workspaceLocked, selectedCwdProp, selectedCwd]);
  ...
  useEffect(() => {
    ...
    if (selectedCwd === null) {
      // When workspace is locked, use the prop directly
      if (workspaceLocked && selectedCwdProp) {
        setSelectedCwd(selectedCwdProp);
        return;
      }
  ...
  }, [allSessions, selectedCwd, initialSessionId, onSelectSession, onInitialRestoreDone, workspaceLocked, selectedCwdProp]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        <div ref={dropdownRef} style={{ position: "relative" }}>
          <button
            onClick={() => { if (!workspaceLocked) setDropdownOpen((v) => !v); }}
            style={{
              width: "100%",
              display: "flex",
              alignItems: "center",
              ...
              borderRadius: 7,
              cursor: workspaceLocked ? "default" : "pointer",
              fontSize: 12,
              color: "var(--text)",
              textAlign: "left",
              opacity: workspaceLocked ? 0.7 : 1,
              transition: "border-color 0.15s, background 0.15s",
            }}
          >
            {workspaceLocked && (
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--text-dim)", flexShrink: 0, marginRight: 4 }}>

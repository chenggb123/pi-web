...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
          ...
          (file) =>
            new Promise<AttachedFile>((resolve, reject) => {
              ...
              reader.onload = () => {
                const result = reader.result as string;
                const base64 = result.split(",")[1];
                resolve({ name: file.name, data: base64, mimeType: file.type, size: file.size });
              };
              reader.onerror = reject;
              reader.readAsDataURL(file);
            })
  ...
  }, [value, attachedImages, attachedFiles, hasAttachments, isStreaming, onSend, clearImages, clearFiles]);
  ...
  const sendQueued = useCallback((mode: "steer" | "followup") => {
    ...
    if (mode === "steer" && onSteer) {
      onSteer(msg, imgs, files);
    } else if (mode === "followup" && onFollowUp) {
      onFollowUp(msg, imgs, files);
    }
    setValue("");
    clearImages();
    clearFiles();
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        {(attachedImages.length > 0 || attachedFiles.length > 0) && (
          <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            {attachedFiles.map((file, i) => (
              <div key={`file-${i}`} style={{
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 500, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 120 }}>
                    {file.name}
                  </div>
          ...
          {isStreaming ? (
            ...
            <button
              ...
              style={{
                ...
                boxShadow: (value.trim() || hasAttachments) ? "0 1px 3px rgba(37,99,235,0.25)" : "none",
              ...
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                ...
                {toolDropdownOpen && (
                  <div style={{
                    ...
                    {TOOL_PRESETS.filter((lvl) => {
                      if (maxToolPreset === "default" && lvl === "full") return false;
                      return true;
                    }).map((lvl) => {
              ...
              <div style={{ position: "relative" }}>
                {compactError && (
                  <div style={{
                    position: "absolute", bottom: "calc(100% + 6px)", right: 0,
                    background: "#1f2937",
                    color: compactError.startsWith("压缩失败") ? "#f87171" : "#9ca3af",

...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const handleSend = useCallback(async (message: string, images?: AttachedImage[], files?: AttachedFile[]) => {
    ...
    const fileBlocks = files?.map((f) => ({ type: "file" as const, name: f.name, mimeType: f.mimeType, size: f.size, data: f.data }));
    const contentBlocks = [
    ...
    const userMsg: AgentMessage = {
      role: "user",
      content: contentBlocks.length > 1 || (contentBlocks.length === 1 && contentBlocks[0].type !== "text")
        ? contentBlocks
        : message,
      timestamp: Date.now(),
    };
    ...
    try {
      if (isNew && newSessionCwd) {
        ...
        onSessionCreated?.({
          ...
          messageCount: 1,
          firstMessage: message || `[${files?.length ?? 0} file(s) uploaded]`,
        });
      } else if (session) {
